...
public class ControllerGame  implements ActionListener{ // interface abstract function
	...
	private void handleLmpLevel1() {
		...
		SwingWorker<Object, Object> sw1 =  new SwingWorker<Object, Object>() {
			...
			protected Object doInBackground() throws Exception {
				playGame(); //yeni thread yaratýyor ve bu sayede gui ve playgame beraber çalýþabiliyor.
				return null;
			}};
		sw1.execute();
	}
	...
	private void playGame() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
		File breakSound = new File("Glass Crush.wav");
		AudioInputStream audioStreambreak = AudioSystem.getAudioInputStream(breakSound);
		Clip clipbreak = AudioSystem.getClip();
		clipbreak.open(audioStreambreak);

		File levelSound = new File("Wood Plank Flicks.wav");
		AudioInputStream audioStreamlevel = AudioSystem.getAudioInputStream(levelSound);
		Clip cliplevel = AudioSystem.getClip();
		cliplevel.open(audioStreamlevel); 
		
		File dieSound = new File("Gunfire And Voices.wav");
		AudioInputStream audioStreamdie = AudioSystem.getAudioInputStream(dieSound);
		Clip clipdie = AudioSystem.getClip();
		clipdie.open(audioStreamdie);
...
//          the block accordingly and also chooses which type of bounce that will be done.
			...
			if(currentBrick != null && currentBrick.getHitLife() > 0) {
				...
				case 1:
					...
					clipbreak.setMicrosecondPosition(0);
					clipbreak.start();
			...
			if(myBall.gety() >= 27) {
				myBall.setx(5);
				myBall.sety(15);
				clipdie.setMicrosecondPosition(0);
				clipdie.start();				
			...
			if(!level.hasBrick()) {
				...
				cliplevel.setMicrosecondPosition(0);
				cliplevel.start();
				myBall.setx(5);
				myBall.sety(15);
				myBall.setlocationchangeX(1);
